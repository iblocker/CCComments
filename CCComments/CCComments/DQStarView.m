...
- (void)creationDQUIFunction{
    ...
    for (int i=0; i<_starTotalCount; i++) {
        ...
        DefaultImageView.frame = CGRectMake(_starSpace*(i+1)+size.width*i,0, size.width, size.height);
    ...
    for (int i=0; i<_starTotalCount; i++) {
        ...
        SelectImageView.frame = CGRectMake(_starSpace*(i+1)+size.width*i, 0, size.width, size.height);
...
- (NSInteger )getEventShowStartForPoint:(CGPoint )point{
    ...
    if (point.x <= size.width + _starSpace) {
        index = floor(point.x/(size.width+_starSpace));
    }
    else {
        index = ceil(point.x/(size.width+_starSpace));
    }
    if (index < 1) {
        index = 1;
    }
